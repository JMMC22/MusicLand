...
function getUsers(req, res) {
    ...
    var itemsPerPage = 5; {
        User.find({ "username": { $ne: "admin" } }).sort('_id').paginate(page, itemsPerPage, (err, users, total) => {
            if (err) return res.status(500).send({ message: 'Error en la petición.' });
...
async function getSimilarPublications(user_id) {
    ...
    similar.forEach((publication) => {
        similarsUsers.push(publication.user);
    });
    ...
    follows.forEach((follow) => {
        var i = similarsUsers.includes(follow.followed);
        if (i !== -1) {
            similarsUsers.splice(i, 1);
