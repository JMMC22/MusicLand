...
function songSave(req, res) {
    ...
    if (!params.titulo || !params.fechaLanzamiento || !params.imagen || !params.artist || !params.url) {
...
function getSongsPagination(req, res) {
    ...
    Song.find().collation({ locale: "en" }).sort('titulo').populate('artist').paginate(page, itemsPerPage, (err, songs, total) => {
        if (err) return res.status(500).send({ message: 'Error en la petición.' });
...
function updateSong(req, res) {
    ...
    if (!update.titulo || !update.fechaLanzamiento || !update.imagen || !update.artist || !update.url) {
        return res.status(200).send({ message: "Enviar datos necesarios." })
    }
...
function deleteSong(req, res) {
    ...
    Song.find({ '_id': songId }).remove(err => {
        ...
        Publication.remove({ "song": songId }).exec();
