...
function artistSave(req, res) {
    ...
    if (!params.nombre || !params.fechaNacimiento || !params.imagen || !params.nacionalidad) {
        return res.status(200).send({ message: "Enviar datos necesarios." })
    }
    ...
    Artist.find({ "nombre": { $regex: artist.nombre, $options: 'i' } }).exec((err, artists) => {
        if (err) return res.status(500).send({ message: 'Error en la petición.' })
        if (artists && artists.length > 0) {
            return res.status(200).send({ message: 'El artista ya está registrado.' })
        } else {
...
function updateArtist(req, res) {
    ...
    if (!update.nombre || !update.fechaNacimiento || !update.imagen || !update.nacionalidad) {
        return res.status(200).send({ message: "Enviar datos necesarios." })
    }

    Artist.find({ "nombre": { $regex: update.nombre, $options: 'i' } }).exec((err, artists) => {
        if (err) return res.status(500).send({ message: 'Error en la petición.' })
        if (artists.length > 0 && artists[0]._id != artistId) {
            return res.status(200).send({ message: 'El artista ya está registrado.' })
        } else {
